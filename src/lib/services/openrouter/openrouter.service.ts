...
${JSON.stringify(existingCards, null, 2)}`;
        ...
        try {
          ...
          const newCard = flashcardResponseSchema.parse(JSON.parse(response.data.choices[0].message.content));

          // Sprawdź czy nie ma duplikatu
          const isDuplicate = results.some((card) => card.front === newCard.front || card.back === newCard.back);
